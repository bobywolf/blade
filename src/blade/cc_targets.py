...
from blade_util import find_relative_path
...
class CcTarget(Target):
    ...
    def __init__(self,
        ...
        self.data['defs'] = defs
        # add by zxy(zhangxinyu@sogou-inc.com) 20140929 support absolute include path (/common/gsl/include)
    ...
    def _get_real_filename(self, path):
        """
            find out the real library file when
            the case of letters or prefix does not match exactly.
            target:libace.a         real:libACE.a
            target:liblibevent.a    real:libevent.a
        """
        ...
        if lower_name[0:6] == 'liblib':
            for f in os.listdir(cur_dir):
                if f[0:3] == 'lib':
                    extend_name = 'lib' + f
                    if extend_name.lower() == lower_name:
                        return os.path.join(cur_dir, f)
        return path
    ...
    def _prebuilt_cc_library(self, dynamic=0):
        ...
        if not allow_only_dynamic:
            src_path = self._get_real_filename(self._prebuilt_cc_library_src_path())
            self._write_rule(
        ...
        if dynamic:
            ...
            prebuilt_src_file = self._prebuilt_cc_library_src_path(
                                            ...
                                            self.name,
                                            dynamic=1)
            prebuilt_src_file = self._get_real_filename(prebuilt_src_file)
            self._write_rule(
                    'Command("%s", "%s", Copy("$TARGET", "$SOURCE"))' % (
                     prebuilt_target_file,
                     prebuilt_src_file))
...
def GLOB(*ss):
    '''expand glob path like GLOB("*.cpp") to real path list ["./a.cpp", "b.cpp", ...]
    '''
    strs = []
    # enter source code directory
    current_source_path = blade.blade.get_current_source_path()
    cur_path = os.getcwd()
    os.chdir(current_source_path)
    for s in ss:
        ps = string.split(s)
        for p in ps:
            strs.extend(glob.glob(p))
