...
def find_relative_path(cur_dir, des_dir):
    '''find a path to dest from current directory
       For exmaple:
           current:   /a/b/c/d
           dest   :   /a/b/e/f/g
           result :   ./../../e/f/g
    ...
    result = "."
    i = len(dirs)
    while des_dir.find("/".join(dirs[:i])) == -1:
        i -= 1
        result += "/.."
    return result + des_dir.replace("/".join(dirs[:i]), "", 1)
